...
1. User's data intended only for that user to use, like file properties and folder names. It can be decrypted using 
`rust_filen::crypto::decrypt_metadata_str_any_key` with user master keys,
and encrypted using `rust_filen::crypto::encrypt_metadata_str` with user's last master key.
...
so receiver's metadata needs to be encrypted with receiver's public key,
so receiver can decipher metadata later with its private key.

3. Linked item metadata is treated like user's private data in list item #1,
only with link key instead of user's last master key.
