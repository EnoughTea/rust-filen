...
// New master key is generated by Filen each time user changes password, so when decrypting previously encrypted
// user data we have to try not only the last master key, but all of the previous keys as well.
let master_keys = login_response_data.decrypt_master_keys_metadata(&last_master_key).unwrap();
```
